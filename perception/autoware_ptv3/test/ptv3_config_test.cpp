...
TEST(PTv3ConfigTest, RejectsPatchSizesThatDoNotCoverEveryStage)
{
  EXPECT_THROW(
    PTv3Config(
      true, false, "", 8, {1, 4, 8}, {-1.0F, -1.0F, -1.0F, 3.0F, 3.0F, 3.0F}, {1.0F, 1.0F, 1.0F},
      {"noise"}, {{"noise", "NOISE"}}, {"z", "z-trans"}, {2, 2}, {8, 16, 32}, {4, 4}, {0, 0, 0}, {},
      "xyzi", false, "partial", {0, 0}),
    std::runtime_error);
  EXPECT_THROW(
    PTv3Config(
      true, false, "", 8, {1, 4, 8}, {-1.0F, -1.0F, -1.0F, 3.0F, 3.0F, 3.0F}, {1.0F, 1.0F, 1.0F},
      {"noise"}, {{"noise", "NOISE"}}, {"z", "z-trans"}, {2, 2}, {8, 16, 32}, {4, 0, 4}, {0, 0, 0},
      {}, "xyzi", false, "partial", {0, 0}),
    std::runtime_error);
}
...
{
  const auto config = makeSegmentationConfig({"noise"}, {{"noise", "NOISE"}});
