...
TEST(TrajectoryModifierUtilsTest, ValidatesTrajectoryPoint)
{
  ...
  EXPECT_TRUE(autoware::trajectory_modifier::utils::validate_point(valid_point));

  TrajectoryPoint invalid_point;
  invalid_point.pose.position.x = std::nan("");
  EXPECT_FALSE(autoware::trajectory_modifier::utils::validate_point(invalid_point));
}
